...
CAS::AbstractExpression::EvalRes EqualityExpression::eval(CAS::Scope &scope, bool lazy) const
{
    ...
    if (firstOpResult.first != secondOpResult.first) return std::make_pair(CAS::TypeInfo::BOOL, make_unique<BoolValue>(false));
        ...
        return std::make_pair(CAS::TypeInfo::BOOL, make_unique<BoolValue>(true));
    else return std::make_pair(CAS::TypeInfo::BOOL, make_unique<EqualityExpression>(std::move(firstOpResult.second), std::move(secondOpResult.second)));
}
