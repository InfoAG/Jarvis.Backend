...
CAS::AbstractExpression::EvalRes LessOrEqualExpression::eval(CAS::Scope &scope, bool lazy) const
{
    ...
    else return std::make_pair(CAS::TypeInfo::BOOL, make_unique<LessOrEqualExpression>(std::move(firstOpResult.second), std::move(secondOpResult.second)));
}

bool LessOrEqualExpression::equals(const CAS::AbstractExpression *other) const
{
